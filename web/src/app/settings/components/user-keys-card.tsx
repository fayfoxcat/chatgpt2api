...
import { Ban, CheckCircle2, Copy, ImageIcon, KeyRound, LoaderCircle, Plus, Trash2 } from "lucide-react";
...
function UsageStats({ item }: { item: UserKey }) {
  const genTotal = item.image_gen_success + item.image_gen_fail;
  const editTotal = item.image_edit_success + item.image_edit_fail;
  const hasUsage = item.total_calls > 0;

  if (!hasUsage) {
    return <span className="text-xs text-stone-400">暂无使用记录</span>;
  }
  ...
  return (
    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-stone-500">
      <span className="flex items-center gap-1">
        ...
        共生成 <strong className="text-stone-700">{item.total_images}</strong> 张
      </span>
      {genTotal > 0 && (
        <span>
          文生图{" "}
          <span className="text-emerald-600">{item.image_gen_success} 成功</span>
          {item.image_gen_fail > 0 && (
            <span className="text-rose-500"> / {item.image_gen_fail} 失败</span>
          )}
        </span>
      )}
      {editTotal > 0 && (
        <span>
          图生图{" "}
          <span className="text-emerald-600">{item.image_edit_success} 成功</span>
          {item.image_edit_fail > 0 && (
            <span className="text-rose-500"> / {item.image_edit_fail} 失败</span>
          )}
        </span>
      )}
      <span>调用 {item.total_calls} 次</span>
    </div>
  );
}
